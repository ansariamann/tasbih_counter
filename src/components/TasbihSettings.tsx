...
import { Menu, Volume2, VolumeX, Hash, Target, Vibrate, X } from "lucide-react";
import { motion, AnimatePresence } from "framer-motion";

const MULTIPLIER_OPTIONS = [1, 2, 3];
const TARGET_PRESETS = [33, 100, 313];

type VibrationStrength = "off" | "light" | "medium" | "strong";
...
interface TasbihSettingsProps {
  ...
  setVibrationStrength: (strength: VibrationStrength) => void;
}
      ...
      <AnimatePresence>
        {isOpen && (
          <>
            ...
            {/* Panel */}
            <motion.div
              className="fixed right-0 top-0 h-full w-80 max-w-[85vw] bg-black/90 backdrop-blur-xl border-l border-gold/20 z-50 overflow-y-auto"
              initial={{ x: "100%" }}
              animate={{ x: 0 }}
              exit={{ x: "100%" }}
            ...
            >
              <div className="p-6">
                ...
                <div className="flex items-center justify-between mb-8">
                  <h2 className="text-xl font-light text-gold tracking-wider">Settings</h2>
                  <button
                    onClick={() => setIsOpen(false)}
                ...
                <div className="mb-6">
                  <div className="flex items-center gap-3 mb-3">
                    <Vibrate className={`w-5 h-5 ${vibrationStrength !== "off" ? "text-gold" : "text-muted-foreground"}`} />
                    <span className="text-sm">Vibration</span>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    {(["off", "light", "medium", "strong"] as VibrationStrength[]).map((strength) => (
                      <button
                        key={strength}
                        onClick={() => setVibrationStrength(strength)}
                ...
                <div className="mb-6">
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    >
                      {!MULTIPLIER_OPTIONS.includes(multiplier) ? `${multiplier}x` : "Custom"}
                    </button>
                  </div>
